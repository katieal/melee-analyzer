...
class MeleeDatabase(object):
    def __init__(self, username, password):
        ...
        self.df = pd.DataFrame.from_records(data=self.read({}), index=['bracket_id'])
        self.df.drop(columns=['_id'], inplace=True)
        self.df_no_index = pd.DataFrame.from_records(data=self.read({}))
        self.df_no_index.drop(columns=['_id'], inplace=True)

    def refresh(self):
        """
        Refresh data in dataframe to sync with MongoDB
        """
        self.df = pd.DataFrame.from_records(data=self.read({}), index=['bracket_id'])
        self.df.drop(columns=['_id'], inplace=True)
        self.df_no_index = pd.DataFrame.from_records(data=self.read({}))
